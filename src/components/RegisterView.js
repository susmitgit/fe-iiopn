...
const RegisterView = (props) => {
	...
	const isDisabled = () => {
		...
		if (state.username === '' || !state.username) {
			if (state.username_error_text != null)
				dispatch(
					authActions.setAuth({
			...
			if (state.username_error_text == null)
				dispatch(
					authActions.setAuth({
						username_error_text: 'Username must be at least 3 characters'
					})
				);
		}
		if (state.email === '') {
	...
	const register = (e) => {
		e.preventDefault();
		dispatch(authActions.registerUser(state.username, state.email, state.password, history));
	};
		...
		<div className="col-md-6 col-md-offset-3" onKeyPress={(e) => _handleKeyPress(e)}>
			<Paper style={style}>
				<div className="text-center">
					...
					{state.registerStatusText && <div className="alert alert-info ">{state.registerStatusText}</div>}
					<div className="col-md-12">
						<TextField
							hintText="Username"
							floatingLabelText="Username"
							type="username"
							errorText={state.username_error_text}
